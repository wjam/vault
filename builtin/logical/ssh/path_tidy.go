...
func (b *backend) pathTidyWrite(
	...
	if tidyCertStore {
		...
		for _, serial := range serials {
			...
			var certificate sshCertificate
			err = certEntry.DecodeJSON(&certificate)
			if err != nil {
				return nil, errutil.InternalError{Err: fmt.Sprintf("Unable to decode entry for SSH certificate serial %s", serial)}
			}

			if time.Now().After(certificate.ValidBefore.Add(bufferDuration)) {
	...
	if tidyRevocationList {
		...
		for _, serial := range revokedSerials {
			...
			var certificate sshCertificate
			err = revokedEntry.DecodeJSON(&certificate)
			if err != nil {
				return nil, errutil.InternalError{Err: fmt.Sprintf("Unable to decode entry for SSH certificate serial %s", serial)}
			}
