...
func (b *backend) pathSignCertificate(req *logical.Request, data *framework.FieldData, role *roleEntry) (*logical.Response, error) {
	...
	entry, err := logical.StorageEntryJSON("certs/"+strconv.FormatUint(certificate.Serial, 16), sshCertificate{
		Certificate: signedSSHCertificate,
	...
	if err != nil {
		return nil, errors.New("Unable to store certificate locally")
	}
	...
	response.Data["signed_key"] = signedSSHCertificate

	return response, nil
}

func (b *backend) calculateValidPrincipals(data *framework.FieldData, role *roleEntry, isHostCertificate bool) ([]string, error) {
	validPrincipals := data.Get("valid_principals").(string)
	if validPrincipals == "" {
		if role.AllowedValidPrincipals == "" {
			return []string{}, nil
		}
	...
	allowedPrincipals := strings.Split(role.AllowedValidPrincipals, ",")
...
func validateValidPrincipal(validPrincipal string, allowedPrincipals []string, role *roleEntry, isHostCertificate bool) bool {
...
func (b *backend) calculateCertificateType(data *framework.FieldData, role *roleEntry) (uint32, error) {
...
func (b *backend) calculateCriticalOptions(data *framework.FieldData, role *roleEntry) (map[string]string, error) {
...
func (b *backend) calculateExtensions(data *framework.FieldData, role *roleEntry) (map[string]string, error) {
...
func (b *backend) calculateTtl(data *framework.FieldData, role *roleEntry) (time.Duration, error) {
...
func createSigningBundle(keyId string, userPublicKey ssh.PublicKey, validPrincipals []string, duration time.Duration, certificateType uint32, sshCertificateBundle signingBundle,
	role *roleEntry, criticalOptions, extensions map[string]string) creationBundle {
